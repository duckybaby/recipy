...
- "imageUrl" in the source object: emit the URL from the page's \`<meta property="og:image">\` tag — sites design og:image to be shareable / hotlinkable, so it's the most reliable image to embed externally. Copy the URL exactly as written in the meta tag: fully-qualified https, no modifications, no decoding, no normalisation. Do not scrape arbitrary <img> tags from the page body. Do not invent URLs. Emit null only when the page genuinely has no og:image. The server validates each URL by fetching it and nulls any URL that returns non-2xx, so don't try to predict which URLs will hotlink — your job is to capture the og:image accurately. Most recipe sites have a working og:image even when they use signed-CDN paths for it.
