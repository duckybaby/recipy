...
- "imageUrl" in the source object: prefer the URL from the page's \`<meta property="og:image">\` tag — sites design og:image specifically to be shareable / hotlinkable, so it's the most reliable image to embed externally. Do NOT scrape arbitrary <img> tags from the page body, and AVOID signed-CDN paths whose URLs encode an expiring or referrer-bound signature: in particular, Simply Recipes' /thmb/<hash>=/ paths, Dotdash Meredith CDN URLs with /filters:.../ segments, NYT Cooking imgix /resize/ variants, and any CDN URL that looks like /thmb/, /scale/, /sig/, or includes a base64-style hash before a path separator. Those routinely 400 on direct hotlink because the signature only validates from within the source page. If the only available image URL looks like one of those signed variants, prefer null — a clean placeholder is better than a broken-image icon. The URL must be the canonical fully-qualified https URL exactly as written in the meta tag — do NOT modify or invent it. Emit null when the page has no usable image.
